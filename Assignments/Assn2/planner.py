...
def value_eval(mdp, P):
    ...
    T1 = T1[mdp.isNonTerminal, :]
    R1 = np.array([mdp.R[i, P[i], :] for i in range(mdp.S)])
    R1 = R1[mdp.isNonTerminal, :]
    
    U = np.sum(np.multiply(T1, R1), axis = 1)
    T1 = T1[:, mdp.isNonTerminal]
    A = np.eye(mdp.S - mdp.numTerm) - mdp.gamma * T1

    V = np.zeros(mdp.S)
    V[mdp.isNonTerminal] = np.linalg.solve(A, U)
